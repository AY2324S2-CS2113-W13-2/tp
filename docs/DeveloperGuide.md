...
```
public void updateManager(Scanner scanner, WeekView weekView, MonthView monthView,
        boolean inMonthView, TaskManager taskManager, int day, int taskIndex, 
...
```
public static void updateTask(LocalDate date, int taskIndex, 
        String newTaskDescription, Scanner scanner,
...
```
public void addManager(Scanner scanner, WeekView weekView, MonthView monthView, 
        boolean inMonthView, String action,
...
#### Method signature:
```
public static void deleteAllTasksOnDate (TaskManager taskManager, 
...
```
public void freeTimesManager(WeekView weekView, MonthView monthView, 
        boolean inMonthView, String day)
        throws TaskManagerException, DateTimeParseException
```
...
```
public List<String> getFreeTimeSlots(List<Task> events, 
...
```
public void printFreeTimeSlots(List<String> freeTimeSlots,
        LocalDate startDate)
```
...
public class TaskManagerException extends Exception {
    ...
    public static void checkIfDateInCurrentWeek(LocalDate date, WeekView weekView) 
            throws TaskManagerException {
...
public class MarkTaskException extends TaskManagerException {
    public static final String TASK_INDEX_OUT_OF_RANGE_FOR_DAY_WITH_TASKS_MESSAGE = 
            "The task index you attempted to " +
    ...
    public static void checkIfTaskIndexIsValidForMarkingTask(List<Task> dayTasks, 
            int taskIndex) throws MarkTaskException {
...
public class StorageFileException extends Exception {
    ...
    public static void checkStorageTextDateFormat(String date) 
...
private static void updateEventLogging(String newTaskDescription,
                                       String oldDescription, Event oldEvent, 
                                       String[] newDatesAndTimes) {
    ...
    logger.log(Level.INFO, "Updating task end date from " + oldEvent.getEndDate()
            + " to: " + newDatesAndTimes[1]);
    logger.log(Level.INFO, "Updating task start time from " + oldEvent.getStartTime() 
            + " to: " + newDatesAndTimes[2]);
    logger.log(Level.INFO, "Updating task end time from " + oldEvent.getEndTime()
            + " to: " + newDatesAndTimes[3]);
}
```
